...
**Раздел:** "Приборы" → Список всех приборов
...
### 4.2 Создание схемы
1. Схемы создаются автоматически пустыми по названию локаций
...
3. Прибор установлен на хосте, перетягиваем в нужное положение
...
Расположение файла БД: `data/kipia_management.db`
...
2. Скопируйте файл `data/kipia_management.db`
...
4. Скопируйте сохраненный файл БД в папку `data/`
...
- Автоматическое определение режима работы (dev/prod)
...
- addDevice(Device device) - добавление прибора
- updateDevice(Device device) - обновление данных
- deleteDevice(int id) - удаление прибора
- getAllDevices() - получение всех приборов
- getDevicesByLocation(String location) - фильтрация по месту
...
- Управление привязками приборов к схемам

#### DeviceLocationDAO

- Привязка приборов к координатам на схемах
