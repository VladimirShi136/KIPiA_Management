...
if not exist "installer_resources\log4j2.xml" (
    echo Creating log4j2.xml for user directory...
    ...
    echo         ^<File name="FileAppender" fileName="${sys:user.home}/AppData/Roaming/KIPiA_Management/logs/kipia_management.log"^> >> "installer_resources\log4j2.xml"
    echo             ^<PatternLayout^> >> "installer_resources\log4j2.xml"
    echo                 ^<Pattern^>%%d{yyyy-MM-dd HH:mm:ss} [%%t] %%-5level %%logger{36} - %%msg%%n^</Pattern^> >> "installer_resources\log4j2.xml"
    echo             ^</PatternLayout^> >> "installer_resources\log4j2.xml"
    echo         ^</File^> >> "installer_resources\log4j2.xml"
    echo     ^</Appenders^> >> "installer_resources\log4j2.xml"
    echo     >> "installer_resources\log4j2.xml"
    echo     ^<Loggers^> >> "installer_resources\log4j2.xml"
    echo         ^<Logger name="com.kipia.management" level="INFO" additivity="false"^> >> "installer_resources\log4j2.xml"
...
if not exist "installer_resources\ViewLogs.bat" (
    echo Creating ViewLogs.bat for user directory...
    echo @echo off > "installer_resources\ViewLogs.bat"
    echo chcp 65001 >> "installer_resources\ViewLogs.bat"
    echo title KIPiA Management System - View Logs >> "installer_resources\ViewLogs.bat"
    ...
    echo echo ======================================== >> "installer_resources\ViewLogs.bat"
    echo echo. >> "installer_resources\ViewLogs.bat"
    echo set "LOG_FILE=%%APPDATA%%\KIPiA_Management\logs\kipia_management.log" >> "installer_resources\ViewLogs.bat"
    echo. >> "installer_resources\ViewLogs.bat"
    ...
    echo ) >> "installer_resources\ViewLogs.bat"
    echo. >> "installer_resources\ViewLogs.bat"
    echo echo Opening log file... >> "installer_resources\ViewLogs.bat"
...
echo Name: "{group}\KIPiA Management System"; Filename: "{app}\KIPiA_Management.bat"; IconFilename: "{app}\iconApp.ico" >> "KIPiA_Setup_Final.iss"
...
echo Name: "{autodesktop}\KIPiA Management System"; Filename: "{app}\KIPiA_Management.bat"; Tasks: desktopicon; IconFilename: "{app}\iconApp.ico" >> "KIPiA_Setup_Final.iss"
echo. >> "KIPiA_Setup_Final.iss"

echo [Run] >> "KIPiA_Setup_Final.iss"
echo Filename: "{app}\KIPiA_Management.bat"; Description: "{cm:LaunchProgram,KIPiA Management System}"; Flags: nowait postinstall skipifsilent >> "KIPiA_Setup_Final.iss"
echo. >> "KIPiA_Setup_Final.iss"
...
if exist "InnoOutput\KIPiA_Management_Setup.exe" (
    ...
    echo - Java requirement check (English message)
